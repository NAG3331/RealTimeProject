...
public class UserRegistrationController {
    ...
    @ApiOperation(value = "Add User Personal details")
    @RequestMapping(value = "/user/userPersonalDetails/{emailId:.+}", method = RequestMethod.POST,
            consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE},
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<User> createUserPersonalDEtails(@PathVariable("emailId") String emailId,
                                           @Valid @RequestBody UserPersonalDetails userPersonalDetails) {
    	User user;
            if (!emailId.equals(userPersonalDetails.getEmailId()))
            {
                throw new MismatchIdentifierException(emailId);
            }
      
            if(userRegistrationService.getUser(emailId)!=null)     
            { 
            
                user= this.userRegistrationService.getUser(emailId);
               user.setUserPersonalDetails(userPersonalDetails); 
              // userRegistrationService.createUser(emailId, user);
               
            }
      
            else
            {
    	        if(userMetaDataService.getUserMetaData(emailId)!=null) 
    	        {
    	  
			    	  UserMetaData userMetaData= userMetaDataService.getUserMetaData(emailId);
			    	   user = new User();
			    	 // userRegistrationService.createUser(emailId, user);
			    	  user.getUserPersonalDetails().setEmailId(emailId);
